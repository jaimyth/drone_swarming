...
n_pp = 200
x_pos = 90
y_pos = np.linspace(40,600, n)
y_pos_pp = np.linspace(20, 100, n_pp)
print(y_pos_pp)
for i in range(n):
...
for i in range(n_pp):
    pp = Ppoint(env, y_pos_pp[i]+100+10*i, y_pos_pp[i]+200)
    env.add_ppoint(pp)
run =  True
...
while run:
    ...
    for i, ag in enumerate(env.agents):
        ag.final_v(cohese=True, avoid=True, align=False, follow=False, go_point=True)
        ag.update_velocity()
        ag.update_position()
        ag.draw_history(env.screen)

    for i, pp in enumerate(env.ppoints):
        pp.draw(env.screen)
