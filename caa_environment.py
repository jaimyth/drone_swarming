...
ag1 = Agent(environment= env, x= 100, y = 180, color = [255,0,0], id=get_id())
ag2 = Agent(environment= env, x= 120, y = 200, color = [255,0,0], id=get_id())
ag3 = Agent(environment= env, x= 100, y = 220, color = [255,0,0], id=get_id())
random.seed(2)
env.add_agent(ag1)
env.add_agent(ag2)
env.add_agent(ag3)
...
while run:
    ...
    for i in env.agents[3:]:
        i.final_v(cohese=True, avoid=True, align=True)
        i.update_velocity()
        i.update_position()
